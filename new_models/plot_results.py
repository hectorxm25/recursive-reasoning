...
def run_figure_2(model_ctx, filename="fig2_wrongness_polarization.png", punishment_mode='mild'):
    ...
    for col, agent_type in enumerate(AGENT_TYPES):
        ...
        run_subplot_wrongness_polarization(model_ctx, ax1, agent_type, agents, colors, pol_values, auth_w=0.8, punishment_mode=punishment_mode)
        if col == 0:
            ax1.legend(loc='upper left', fontsize=7)
        ...
        run_subplot_wrongness_polarization(model_ctx, ax2, agent_type, agents, colors, pol_values, auth_w=0.2, punishment_mode=punishment_mode)
...
def run_figure_3(model_ctx, filename="fig3_trust_polarization.png", punishment_mode='mild'):
    ...
    for col, agent_type in enumerate(AGENT_TYPES):
        ...
        ax1 = axes[0, col]
        ax1.set_title(f"{agent_type['label']}\nAuthority: W=0.8 (Wrong)", fontsize=10)
        run_subplot_trust_polarization(model_ctx, ax1, agent_type, agents, colors, pol_values, auth_w=0.8, punishment_mode=punishment_mode)
        if col == 0:
            ax1.legend(loc='upper left', fontsize=7)
        ...
        ax2 = axes[1, col]
        ax2.set_title(f"Authority: W=0.2 (Not Wrong)", fontsize=10)
        run_subplot_trust_polarization(model_ctx, ax2, agent_type, agents, colors, pol_values, auth_w=0.2, punishment_mode=punishment_mode)
